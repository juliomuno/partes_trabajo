...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	$insp_izacar = $_REQUEST['chk_izacar'];
	if ($insp_izacar =='') {
		$insp_izacar = 0;
	}

	$insp_traalt = $_REQUEST['chk_traalt'];
	if ($insp_traalt =='') {
		$insp_traalt = 0;
	}

	$insp_traten = $_REQUEST['chk_traten'];
	if ($insp_traten =='') {
		$insp_traten = 0;
	}
	...
	if (!$respuesta->error) {
		if ($op == "C") {
			$sSQL = "INSERT INTO INSP (INSP_ID, INSP_FEC, INSP_PRE, INSP_USR, INSP_TIP_OBR, INSP_TIP_TRA, INSP_DIR, INSP_POB, INSP_TRA_DES, INSP_OBS, INSP_CSSDF, INSP_SORP, INSP_NSORP, INSP_LAT, INSP_LON, INSP_USU, INSP_VEH, INSP_DES_INC, INSP_COBS, INSP_CINC, INSP_EXT1, INSP_EXT2, INSP_JEF, INSP_RPR, INSP_IZACAR, INSP_TRAALT, INSP_TRATEN, INSP_MOD) ";
			$sSQL .= "VALUES (";
			$sSQL .= $insp_id;
			$sSQL .= ", " . $insp_fec;
			$sSQL .= ", " . $insp_pre;
			...
			$sSQL .= "INSP_IZACAR=" . $insp_izacar . ",";
			$sSQL .= "INSP_TRAALT=" . $insp_traalt . ",";
			$sSQL .= "INSP_TRATEN=" . $insp_traten . ",";
			$sSQL .= "INSP_MOD=1 ";
