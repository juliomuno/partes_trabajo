  ...
  */
  ...
  if ($op != 'C') {
    ...
    if (mysql_num_rows($sentencia) == 1) {
      ...
      $insp_izacar = $row['INSP_IZACAR'];

      $insp_traalt = $row['INSP_TRAALT'];

      $insp_traten = $row['INSP_TRATEN'];
...
if ($insp_traalt == 1){
    $saux .= ' Trabajo en altura';
}

if ($insp_traten == 1){
    $saux .= ' Trabajo en tensión';
}
...
$operarios_subc='<table width="300px">';

$subc_nom='';

$sentencia = DB_CONSULTA("SELECT ISO_NOM, IS_NOM FROM INSP_OPE_SUBC INNER JOIN INSP_SUBC ON (INSP_OPE_SUBC.IOS_SUBC=INSP_SUBC.IS_ID) INNER JOIN INSP_SUBC_OPE ON (IOS_OPE=ISO_ID) where IOS_INSP=" . $insp_id . ";");

  while ($row = mysql_fetch_assoc($sentencia)) {

    //$subc_nom .= $row['IS_NOM'];

    $operarios_subc .= '<tr><td>' . $row['IS_NOM'] . '</td><td>' . $row['ISO_NOM'] . '</td>';

  }

$operarios_subc .= '</table>';

$content .= '<div class="form-group">';

$content .= '<div class="form-group"><label for="vehiculo" class="col-lg-2">Subcontrata:</label>';

$content .= '<div class="col-lg-10">';    

$content .= $operarios_subc;
...
if ($frm_id!=0){
    ...
    $sql = "SELECT FRM.FRM_NOM, INSP_RESP.*, PREG_ID, PREG_ORD, PREG_NOM, PREG_CAT.* FROM INSP_RESP INNER JOIN PREG ON (INSP_RESP.IR_PREG=PREG.PREG_ID) INNER JOIN PREG_CAT ON (PREG.PREG_CAT=PREG_CAT.PC_ID) INNER JOIN FRM ON (FRM.FRM_ID=INSP_RESP.IR_FRM) WHERE IR_INSP=" . $insp_id . " AND IR_FRM= " . $frm_id . " AND IR_PREG=PREG_ID AND PREG_CAT = PREG_CAT.PC_ID ORDER BY PC_ORD, PREG_ORD";
    ...
    if (mysql_num_rows($sentencia) >= 1) {
      ...
      $row = mysql_fetch_assoc($sentencia);
        ...
        $content .= '<h3 align="center">Formulario ' . $row['FRM_NOM'] . '</h3>';
    ...
    while ($row = mysql_fetch_assoc($sentencia)) {
      ...
      if ($row['IR_IMG'] == '') {
        ...
        $content .= '<p><img src="'.$imagen_preliminar.'"/></p>';
...
$content .= '<script type="text/javascript">javascript:window.print();</script>';
