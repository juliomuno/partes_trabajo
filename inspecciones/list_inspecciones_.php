...
<body class="framework">
    ...
    <table class="table" style="margin:5px;">
      ...
      $sql = "SELECT * FROM LIST_INSPECCIONES_ORD_ZA";
      $sql .= " WHERE Fecha>='" . strftime("%Y") . "-01-01'";
      if ($_SESSION['GLB_USR_ID']!=962){
        $sql .= " AND Usuario=" . $_SESSION['GLB_USR_ID'];
        }
        ...
        <tr>
            <td class="detalle_fila" onclick="javascript: window.location.href='inspeccion.php?op=M&id=<?php echo $row['INSP_ID']?>';">
                <table style="width:100%;">
                    <tr>
                        <td class="id_registro"><strong><?php echo $row['INSP_ID'];?>
                        <?php
                        if ($_SESSION['GLB_USR_ID']==962){ 
                          //echo ' ('.$row['Usuario'].')';
                          echo ' (' . DB_LEE_CAMPO("USR_WEB", "CONCAT_WS(' ',USR_NOM,USR_APE)", "USR_ID=" . $row['Usuario']) . ')';
                        }
                        ?>
                        </td>
            ...
            <td class="detalle_fila" onclick="javascript: window.location.href='inspeccion_pdf.php?op=M&id=<?php echo $row['INSP_ID']?>';">                
