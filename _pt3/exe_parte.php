...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if (!$respuesta->error) {
		if ($op == "C") {
			$sSQL = "INSERT INTO PAR (PAR_ID, PAR_TIP, PAR_FEC_REA, PAR_FEC, PAR_HINI, PAR_HFIN, PAR_NINC, PAR_FIN, PAR_UOT, PAR_DIR, PAR_POB, PAR_REA, PAR_OBS, PAR_TIP_AVE, PAR_VEH, PAR_KM, PAR_KM_HOR, PAR_MOD) ";
			$sSQL .= "VALUES (" . $par_id . ", " . $par_tip . ", " . $par_fec_rea . ", " . $par_fec . ", " . $par_hini . ", " . $par_hfin . ", " . $par_ninc . ", " . $par_fin . ", " . $par_uot . ", " . $par_dir . ", " . $par_pob . ", " . $par_rea . ", " . $par_obs . ", " . $par_tip_ave . ", " . $par_veh . ", " . $par_km . ", " . $par_km_hor . ", 1)";
	...
	*/
	...
	if (!$respuesta->error && $op == "C") {
		...
		for($i=1;$i<=$num_trabajadores;$i++) {	
	    	if (!$respuesta->error) {
				...
				$pd_ope = $_REQUEST["txt_ope" . $i];
				$pd_hnor = $_REQUEST["txt_hnor_" . $i];
				$pd_hext = $_REQUEST["txt_hext" . $i];
				$pd_hnoc = $_REQUEST["txt_hnoc" . $i];
				$pd_cond = $_REQUEST["txt_ope_sel" . $i];

				if ($pd_ope != "") {
					$sSQL = "INSERT INTO PAR_DET (PD_PAR, PD_OPE, PD_NOR, PD_EXT, PD_NOC, PD_CON, PD_DESP, PD_HINI, PD_HFIN) VALUES ";
					...
					$sSQL .= "(" . $par_id . "," . $pd_ope . "," . $pd_hnor . "," . $pd_hext . "," . $pd_hnoc . "," . $pd_cond . "," . $pd_hdes . "," . $par_hini . "," . $par_hfin . ")";
					$resultado = DB_EJECUTA($sSQL);
	...
	foreach($_FILES["txt_img"]['tmp_name'] as $key => $tmp_name){
		...
		if($_FILES["txt_img"]['name'][$key]) {
			...
			$ruta = "/_pt2/partes_imagenes/".uniqid(rand(10,1000), true).'_'.$_FILES["txt_img"]["name"];

			//Movemos y validamos que el archivo se haya cargado correctamente
			//El primer campo es el origen y el segundo el destino
			move_uploaded_file($source, $ruta);
		...
		if ($op == "C") {
			...
			for ($i=0;$i<$max;$i++){
				if ($ope_sel_reg[$i]!=""){
					...
					$hnc=$_REQUEST['txt_chk_hnc'.$ope_sel_reg[$i]];
					...
					$sSQL = "UPDATE USR_JOR SET UJ_FEC_FIN=" . $par_fec_fin . ", UJ_HNOR=" . str_replace(",", ".", $hno) . ", UJ_HEXT=" . str_replace(",", ".", $hex) . ", UJ_HDES=" . str_replace(",", ".", $hde) . ", UJ_HNOC=" . str_replace(",", ".", $hnc) . ", UJ_MOD=1 WHERE UJ_USU=" . $ope . " AND UJ_JOR=0 AND UJ_TIP_STOP=1 AND UJ_FEC_FIN IS NULL AND UJ_FEC_INI>=" . $fecha_dia_anterior;
					$resultado = DB_EJECUTA($sSQL);
	...
	/** -------- Para finalizar el trabajo en parte ---------- */
	...
	if (!$respuesta->error) {
		// Eliminamos las imágenes obtenidas en la tabla PAR_IMG más recientes y añadimos las nuevas con nombre único y con valor de PI_PRL = 1
		for ($i = 0; $i < count($imagenes); $i++) { 
			$sSQL = "DELETE FROM PAR_IMG WHERE PI_PRL = 1 AND PI_PAR=" . $par_id . ";";
			$resultado = DB_EJECUTA($sSQL);
			
			if (!$resultado) {
				$respuesta->error = true;
				$respuesta->mensaje = "Error al eliminar imágen de la tabla PAR_IMG." . $sSQL;
			}
		}
		...
		foreach($_FILES["archivo"]['tmp_name'] as $key => $tmp_name){
			...
			if($_FILES["archivo"]['name'][$key]) {
				...
				$ruta = "/_pt2/partes_imagenes/".uniqid(rand(10,1000), true).'_'.$_FILES["archivo"]["name"];
