...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	*/
	...
	/** Para subir nuestras imágenes a la base de datos */

	if (!$respuesta->error) {
		// Múltiples imágenes del menú principal
		
		foreach ($_FILES["archivo"]['tmp_name'] as $key => $tmp_name) {
			// Validamos que el archivo exista en el input de subir imágenes
			if ($_FILES["archivo"]["name"][$key]) {
				$filename = $_FILES["archivo"]["name"][$key];

				// Validamos que se suban imágenes no duplicadas a la base de datos
				$existeImagen = "SELECT PI_IMG FROM PAR_IMG_TEMP WHERE PI_IMG = $filename;";

				if ($existeImagen != null) {
					// Subimos las imágenes a la carpeta /_pt3/partes_imagenes (que en el proyecto real será otro directorio)
					
					$source = $_FILES["archivo"]["tmp_name"][$key];
					$ruta = "/_pt3/partes_imagenes/".uniqid(rand(10,1000), true).'_'.$_FILES["archivo"]["name"];
					
					// Realizamos la acción de subirlas a la carpeta
					move_uploaded_file($source, $ruta);

					if ($filename != '') {
						// Le asignamos nombre único a la imágen
						$nuevo_nombre_imagen = uniqid(); 

						// Insertamos las imágenes en la base de datos (de la tabla en sucio)
						$sSQL = "INSERT INTO PAR_IMG_TEMP (PI_IMG, PI_PEE) VALUES (" . STR_formato_cadena($nuevo_nombre_imagen.".jpg") . "," . $pla_pee . ");";
						$resultado = DB_EJECUTA($sSQL);

						if (!$resultado) {
							$respuesta->error = true;
							$respuesta->mensaje = "Error al almacenar imágen en Comercial Moneleg S.L. partes de trabajo." . $sSQL;
							break;
						}
					}	
				}
					...
					$respuesta->error = true;
					$respuesta->mensaje = "Error. Imágen/es ya contemplada/s en partes de trabajo.";
					break;
				}
			}
		}
