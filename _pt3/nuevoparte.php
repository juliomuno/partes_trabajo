...
<html lang="es">
      ...
      if ($op != 'C') {
        ...
        if (mysql_num_rows($sentencia) == 1) {
          ...
          while ($row = mysql_fetch_assoc($sentencia_ope)) {
            ...
            $hor_dif=$row['PD_NOR']+$row['PD_EXT']+$row['PD_NOC'];
            $i++;
          }
          ...
          $pee_id = DB_LEE_CAMPO("USR_JOR","UJ_PEE","UJ_PAR=" . $id);
  ...
  <script type="text/javascript">
      ...
      function horas_normales(){
        ...
        if (document.formulario.txt_hnoc1.value.length==0){
          noc=0;
        } else {
          noc=document.formulario.txt_hnoc1.value;
        }
        ...
        //  document.getElementById("modal_text").innerHTML = "La suma de Extras y Nocturnas no pueden ser superior a: " + hor;
        //  $("#myModal").modal("show");
        //  return 0;
        }
      ...
      function validar_formulario(){ 
        ...
        if (document.formulario.txt_ninc.value.length > 10) {
          msg += "<p>El N&deg; de Incidencia no puede contener más de 10 caracteres.</p>";
        }
...
<body class="framework" onload="javascript: marcar_conductores();">
  <div class="container">
    ...
    <input type="hidden" style="width: 1000px;" name="txt_des_hor" id="txt_des_hor" value="<?php echo $des_hor; ?>">
      ...
      <div class="bloque">
        <div class="form-group bloque-reducido">
          ...
          <div class="col-lg-10">
            <?php 
              ...
              ?>
              <input type="hidden" id="txt_ope<?php echo $i ?>" name="txt_ope<?php echo $i ?>" value="<?php echo $par_ope[($i-1)]['pd_ope'] ?>">
              <input type="hidden" name="txt_ope_sel<?php echo $i ?>" id="txt_ope_sel<?php echo $i ?>">
          </div>
        </div>
        ...
        <div class="form-group bloque-reducido">
          <div class="col-xs-6 bloque">
            ...
            <input type="number" class="form-control decimal" id="txt_hnor<?php echo $i?>" name="txt_hnor<?php echo $i?>" disabled="disabled" value="<?php echo $par_ope[$i-1]['pd_nor'];?>" />
            <input type="hidden" id="txt_hnor_<?php echo $i?>" name="txt_hnor_<?php echo $i?>" value="<?php echo $par_ope[$i-1]['pd_nor'];?>" />
          </div>
    
          <div class="col-xs-6 bloque">
            <label for="horas_extras<?php echo $i ?>" class="">Extras:</label>
              <input type="number" class="form-control decimal" id="txt_hext<?php echo $i ?>" name="txt_hext<?php echo $i ?>"  <?php echo $horas_disabled;?>  onchange="javascript:horas_normales();" value="<?php echo $par_ope[$i-1]['pd_ext'];?>" />
          </div>

          <div class="col-xs-6 bloque">
            <label for="horas_nocturnas<?php echo $i ?>" class="">Nocturnas<?php echo $sql_aux;?>:</label>
            <input type="number" class="form-control col-xs-12 decimal" id="txt_hnoc<?php echo $i ?>" <?php echo $horas_disabled;?>  name="txt_hnoc<?php echo $i ?>" onchange="javascript:horas_normales();" value="<?php echo $par_ope[$i-1]['pd_noc'];?>" />
          </div>
        </div>
        ...
        if ($pee_id!=""){
          ...
          <div class="col-lg-10">
            ...
            <input type="hidden" name="txt_pla_ope_sel" id="txt_pla_ope_sel" >
              <?php
              echo DB_LIST_CHECK_PARTES("UJ_PLA_JEF=0 AND UJ_FEC_FIN IS NULL AND UJ_FEC_INI>=CURDATE()-1 AND UJ_PEE=".$pee_id,"","","onchange=javascript:registrar_seleccion_opes(this.value,this.checked);","",$op,1);
             } else {
              ?>
            <label for="ope_sel<?php echo $i ?>" class="">Miembros del equipo:</label>
            <input type="hidden" name="txt_pla_ope_sel" id="txt_pla_ope_sel" >
              <?php
              echo DB_LIST_CHECK_PARTES("UJ_PLA_JEF=0 AND UJ_PEE=".$pee_id,"","","onchange=javascript:registrar_seleccion_opes(this.value,this.checked);","",$op,1);
            }
            ?>
          </div>
          <?php
