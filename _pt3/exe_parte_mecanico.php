...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
	...
	if (!$respuesta->error) {
		if ($op == "C") {
			$sSQL = "INSERT INTO PAR (PAR_ID, PAR_TIP, PAR_FEC, PAR_FEC_REA, PAR_DIR, PAR_POB, PAR_REA, PAR_OBS, PAR_VEH, PAR_TIP_MEC, PAR_HINI, PAR_HFIN, PAR_MOD) ";
			$sSQL .= "VALUES (" . $par_id . ", " . $par_tip . ", " . $par_fec . ", " . $par_fec_rea . ", " . $par_dir . ", " . $par_pob . ", " . $par_rea . ", " . $par_obs . ", " . $par_veh . "," . $par_tip_mec . ", " . $par_hini . ", " . $par_hfin . ",1)";
		} else {
			$sSQL = "UPDATE PAR SET ";
		...
		*/
		...
		if (!$respuesta->error && $op == "C") {
			...
			$sSQL .= "(" . $par_id . "," . $ope1 . "," . $pd_hnor . "," . $pd_hext . "," . $pd_cond1 . "," . $pd_hdes . "," . $par_hini . "," . $par_hfin . ")";
			$resultado = DB_EJECUTA($sSQL);
		...
		if (!$respuesta->error) {
			...
			foreach($_FILES["txt_img4"]['tmp_name'] as $key => $tmp_name){
				//Validamos que el archivo exista
				if($_FILES["txt_img4"]['name'][$key]) {
					$filename = $_FILES["txt_img4"]['name'][$key]; //Obtenemos el nombre original del archivo
					$source = $_FILES["txt_img4"]['tmp_name'][$key]; //Obtenemos un nombre temporal del archivo
					$ruta = "/_pt2/partes_imagenes/".uniqid(rand(10,1000), true).'_'.$_FILES["txt_img4"]["name"];
	...
	/** -------- Para finalizar el trabajo en parte de mecánico ---------- */
