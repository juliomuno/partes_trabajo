...
<html lang="es">
    ...
    <?php
      ...
      if ($op != 'C') {
        ...
        if (mysql_num_rows($sentencia) == 1) {
          ...
          /*
          $i = 0;
          $sentencia_veh = DB_CONSULTA("SELECT * FROM PAR_VEH WHERE PV_PAR=" . $id);
          $par_veh = array();
          while ($row = mysql_fetch_assoc($sentencia_veh)) {
            $par_veh[$i] = array(
              'pd_veh' => $row['PV_VEH'],
              'pd_ope' => $row['PV_OPE'],
              'pd_km' => $row['PV_KM'],
              'pd_km_hor' => $row['PV_KM_HOR']
            );
          }*/
        ...
        if (mysql_num_rows($sentencia) == 1) {
          ...
          $par_veh = array();
          $par_veh[0] = array(
            'pv_veh' => $row['USR_VEH_HAB'],
            'pv_ope' => $_SESSION['GLB_USR_ID'],
            'pv_km' => '',
            'pv_km_hor' => '');
        }
      }
  ...
  <script type="text/javascript">
      ...
      // Vehículos
      function vehiculos_add() {
        var cadena;

        <?php $num_vehiculos++;?>
        cadena = '<div class="bloque">';
        cadena += '<div class="form-group bloque-reducido">';
          cadena += '<label for="vehiculo" class="col-lg-2">Veh&iacute;culo:</label>';
          cadena += '<div class="col-lg-10">';
          cadena += '<?php echo DB_COMBOBOX("LIST_VEHICULOS","Codigo","Nombre","","Nombre","cmb_veh[" . $num_vehiculos . "]","cmb_veh[]","form-control",$par_veh,"","",""); ?>';
          cadena += '</div>';
        cadena += '</div>';
        cadena += '<div class="form-group bloque-reducido">';
          cadena += '<label for="conductor" class="col-lg-2">Conductor:</label>';
          cadena += '<div class="col-lg-10">';
          cadena += '<?php echo DB_COMBOBOX("LIST_OPERARIOS","Codigo","Nombre","","Nombre","cmb_cond[" . $num_vehiculos . "]","cmb_cond[]","form-control","","","",""); ?>';
          cadena += '</div>';
        cadena += '</div>'
        cadena += '<div class="form-group">';
          cadena += '<label for="km" class="col-xs-2 col-xs-offset-1">KM:</label>';
          cadena += '<div class="col-xs-3">';
          cadena += '<input type="number" class="form-control decimal" name="txt_km[]" value="<?php echo $par_km;?>" />';
          cadena += '</div>'
          cadena += '<label for="desplazamiento" class="col-xs-2">Horas:</label>';
          cadena += '<div class="col-xs-3">';
          cadena += '<input type="number" class="form-control decimal" name="txt_hor_km[]" value="<?php echo $par_km_hor;?>"  />';
          cadena += '</div>';
        cadena += '</div>';
        cadena += '<button type="button" class="btn btn-danger" onclick="javascript: vehiculos_delete(this);">Eliminar Vehículo</button></div>';
        cadena += '</div>';
        //cadena = '<div><p>Párrafo Añadido</p><button type="button" class="btn btn-danger" onclick="javascript: vehiculos_delete(this);">Eliminar</button></div>';

        // Añadir hijo a capa vehículos
        $("#capa_vehiculos").append(cadena);
      }

      function vehiculos_delete(vehiculo_eliminar) {
        $(vehiculo_eliminar).parent().remove();
      }
      ...
      function validar_formulario(){ 
        ...
        // Variables boobleanas para no repetir los mensajes
        var berror_conductor = false;
        var berror_vehiculo = false;
        var berror_km = false;
        var berror_hor_km = false;

        // Guardar Operarios en Array
        $("select[name*='cmb_ope']").each(function () {
          if ($(this).val())
            vope.push($(this).val());  
        });
        ...
        if (document.formulario.txt_hnor1.value.length == 0 && document.formulario.txt_hext1.value.length == 0 && document.formulario.txt_hnoc1.value.length == 0) {
          msg += "<p>Es necesario indicar las horas del operario 1.</p>";
        } else if (isNaN(document.formulario.txt_hnor1.value) && !isNaN(document.formulario.txt_hext1.value) && !isNaN(document.formulario.txt_hnoc1.value)) {
          msg += "<p>Es necesario indicar las horas del operario 1.</p>";
        }
        ...
        // Verificar conductores de vehículos
        $("select[name*='cmb_cond'] option:selected").each(function () {
          if ($(this).val()) {
            if (vope.indexOf($(this).val())<0) {
              msg += "<p>No se encontró al conductor '" + $(this).text() + "' en la lista de operarios.</p>";
            }
          }
        });
        
        // Verificamos vehículos
        $("select[name*='cmb_veh']").each(function () {
          if (!$(this).val()) {
            if (!berror_vehiculo) {
              msg += "<p>Existen vehículos que no están definidos en la lista de vehículos.</p>";
              berror_vehiculo = true;
            }
          }
        });
        
        // Verificamos conductores
        $("select[name*='cmb_cond']").each(function () {
          if (!$(this).val()) {
            if (!berror_conductor) {
              msg += "<p>Existen vehículos que no tienen asignado un conductor.</p>";
              berror_conductor = true;
            }
          }
        });

        // Verificamos km Vehículos
        $("input[name*='txt_km']").each(function () {
          if (!$(this).val()) {
            if (!berror_km) {
              msg += "<p>Es necesario indicar los km de los vehículos indicados.</p>";
              berror_km = true;  
            }
            
          } else if (isNaN($(this).val().replace(",","."))) {
            msg += "<p>Los km de los vehículos deben ser numéricos.</p>";
          }
        });

        // Verificamos km Vehículos
        $("input[name*='txt_hor_km']").each(function () {
          if (!$(this).val()) {
            if (!berror_hor_km) {
              msg += "<p>Es necesario indicar el tiempo de desplazamiento de los vehículos indicados.</p>";
              berror_hor_km = true;
            }
          } else if (isNaN($(this).val().replace(",","."))) {
            msg += "<p>El tiempo de desplazamineto de los vehículos debe ser numérico.</p>";
          }
        });
        ...
        if (msg != "") {
          ...
          uploadAjax("exe_parte_vehiculos.php");
        }
      } 
...
<body class="framework">
  <div class="container">
    <form name="formulario" id="formulario" class="form-horizontal" role="form" method="POST" action="exe_parte_vehiculos.php" enctype="multipart/form-data" onsubmit="return false;">
    ...
    <div id="nuevoparte1">
      ...
      <div class="bloque">
        <div class="form-group bloque-reducido">
          ...
          <div class="col-lg-10">
            <?php 
              $cadena_onchange = "javascript: cargar('nuevoparte_materiales.php?par_id=" . $par_id . "'";
        ...
        <div class="col-xs-4 bloque">
          <label for="horas_normales<?php echo $i?>" class="">Normales:</label>
        ...
        <div class="col-xs-4 bloque">
          <label for="horas_nocturnas<?php echo $i ?>" class="">Nocturnas:</label>
          <input type="number" class="form-control col-xs-12 decimal" id="txt_hnoc<?php echo $i ?>" name="txt_hnoc<?php echo $i ?>" value="<?php echo $par_ope[$i-1]['pd_noc'];?>" />
        </div>
    ...
    <div id="nuevoparte2" style="display:none">
      ...
      <div class="bloque">
        ...
        <div class="form-group">
          ...
          <div class="col-lg-10">
            <?php echo DB_COMBOBOX("LIST_TIPOS_AVERIAS","Codigo","Nombre","","Nombre","cmb_tip_ave","cmb_tip_ave","form-control",$par_tip_ave,"","","javascript: cargar('nuevoparte_trabajos_averias.php?par_id=" . $id . "&par_tip_ave=' + this.value, 'capa_trabajos_averias');");?>
          </div>
        </div>
    ...
    <div id="nuevoparte3" style="display:none">
      ...
      <div id="capa_trabajos_averias">
        <?php echo DB_LIST_CHECK("LIST_TRABAJOS_AVERIAS","Codigo","Nombre","Tipo=" . $par_tip_ave,"Nombre","chk_tip_tra[]","", $par_tra);?>
      </div>
    </div>      
    <!-- Fin Nuevo Parte 3/7 -->
    ...
    <div id="nuevoparte5" style="display:none">
      ...
      <div id="capa_vehiculos">
        
        <?php for ($i=0; $i<count($par_veh); $i++) {
          echo '<div class="form-group bloque-reducido">';
          echo '<label for="vehiculo" class="col-lg-2">Veh&iacute;culo:</label>';
          echo '<div class="col-lg-10">';
            echo DB_COMBOBOX("LIST_VEHICULOS","Codigo","Nombre","","Nombre","cmb_veh[" . ($i+1) . "]","cmb_veh[]","form-control",$par_veh[$i]['pv_veh'],"","","");
          echo '</div>';
        echo '</div>';
        echo '<div class="form-group bloque-reducido">';
          echo '<label for="conductor" class="col-lg-2">Conductor:</label>';
          echo '<div class="col-lg-10">';
            echo DB_COMBOBOX("LIST_OPERARIOS","Codigo","Nombre","","Nombre","cmb_cond[" . ($i+1) . "]","cmb_cond[]","form-control",$par_veh[$i]['pv_ope'],"","","");
          echo '</div>';
        echo '</div>';
        echo '<div class="form-group">';
          echo '<label for="km" class="col-xs-2 col-xs-offset-1">KM:</label>';
          echo '<div class="col-xs-3">';
            echo '<input type="number" class="form-control decimal" name="txt_km[]" value="' . $par_veh[$i]['pv_km'] . '" />';
          echo '</div>';
          echo '<label for="desplazamiento" class="col-xs-2">Horas:</label>';
            echo '<div class="col-xs-3">';
              echo '<input type="number" class="form-control decimal" name="txt_hor_km[]" value="' . $par_veh[$i]['pv_km_hor'] . '"  />';
            echo '</div>';
      ...
      <button type="button" class="btn btn-primary" onclick="javascript: vehiculos_add();">Añadir Vehículo</button>
