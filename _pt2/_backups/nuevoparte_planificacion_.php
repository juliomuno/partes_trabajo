      ...
      <div class="bloque">
        <div class="form-group bloque-reducido">
          ...
          <div class="col-lg-10">
            <?php 
              ...
              echo DB_COMBOBOX("LIST_OPERARIOS","Codigo","Nombre","","Nombre","cmb_ope" . $i,"cmb_ope" . $i,"form-control",$_SESSION['GLB_USR_ID'],"","1",$cadena_onchange);
              echo '<input type="hidden" name="txt_ope_sel' . $i . '" id="txt_ope_sel' . $i . '" value="' . $_SESSION['GLB_USR_ID'] . '">';
               ?>
          </div>
        </div>
        ...
        <div class="col-xs-6 bloque">
          <label for="horas_extras<?php echo $i ?>" class="">Extras:</label>
            <input type="number" class="form-control decimal" id="txt_hext1" name="txt_hext1" onchange="javascript:horas_normales();" value="<?php echo $par_ope[$i-1]['pd_ext'];?>" />
        </div>
        ...
        if ($pee_id!="" && $pee_id!=-1){
          ...
          <div class="col-lg-10">
            ...
            <?php 
            //echo DB_LIST_CHECK("USR_JOR INNER JOIN USR_WEB ON UJ_USU=USR_ID","UJ_USU","CONCAT_WS(' ',USR_NOM,USR_APE)","UJ_PEE=".$pee_id." AND UJ_PLA_JEF=0 AND UJ_FEC_FIN IS NULL AND UJ_FEC_INI>=CURDATE()","","chk_pla_ope","","","onchange=javascript:registrar_seleccion_opes(this.value,this.checked);");
            echo DB_LIST_CHECK_PARTES("UJ_PLA_JEF=0 AND UJ_FEC_FIN IS NULL AND UJ_FEC_INI>=CURDATE() AND UJ_PEE=".$pee_id,"","","onchange=javascript:registrar_seleccion_opes(this.value,this.checked);");
            ?>
          </div>
          <?php
          }
