...
$content .= '</div>';
    ...
    $content .= '<h3 align="center">Formulario Prevención</h3>';
    ...
    //MEDIOAMBIENTE
    $content .= '<div style="page-break-before: always" id="nuevoparte3">';
    $content .= '<h3 align="center">Formulario Medioambiente</h3>';
    $content .= '<div class="row"> </div> <div class="row"> </div>';
    $content .= '</div>';
    $sql = "SELECT INSP_RESP.*, PREG_ID, PREG_ORD, PREG_NOM, PREG_CAT.* FROM INSP_RESP, PREG, PREG_CAT WHERE IR_INSP=" . $insp_id . " AND IR_FRM= 3 AND IR_PREG=PREG_ID AND PREG_CAT = PREG_CAT.PC_ID ORDER BY PC_ORD, PREG_ORD";
    $sentencia = DB_CONSULTA($sql);
    $i = 1;
    while ($row = mysql_fetch_assoc($sentencia)) {
      if ($nombre_categoria != $row['PC_NOM']) {
