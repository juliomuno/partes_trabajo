			...
			if ($_FILES["archivo"]["name"][$key]) {
				...
				if ($existeImagen != null) {
					...
					if ($filename != '') {
						...
						// Subimos las imágenes a la carpeta /partes_imagenes
						if (move_uploaded_file($source, "../partes_imagenes/" . $filename . ".jpg")) {	
							...
							$sSQL = "INSERT INTO PAR_IMG_TEMP (PI_IMG, PI_PEE) VALUES (" . STR_formato_cadena($filename.".jpg") . "," . $pla_pee . ")";
							$resultado = DB_EJECUTA($sSQL);

							if (!$resultado) {
								$respuesta->error = true;
								$respuesta->mensaje = "Error al almacenar imágen en Comercial Moneleg S.L. partes de trabajo.";
								break;
							}
						} else { 
							$respuesta->error = true;
